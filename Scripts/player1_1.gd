...
var has_sword = true  # O jogador já começa com a espada
	...
	if Input.is_action_just_pressed("player_interact"):
		if $RayCast2D.is_colliding():
			...
			if collider is Chest and collider.is_closed(): 
				collider.open()
				# O item do baú não altera mais o estado da espada

	if not is_attacking and has_sword and Input.is_action_just_pressed("player attack"):
		attack()
...
func _on_hurt_box_body_entered(body: Node2D) -> void:
	if body is Ghost:
		body.die()
	elif "take_damage" in body:
		body.take_damage(1)


func _on_hit_box_body_entered(body: Node2D) -> void:
	if body is Ghost:
		var hp_control = get_node("/root/Chapter_2/CanvasLayer/HP2")
